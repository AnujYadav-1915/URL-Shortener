...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Welcome to Vynkify, ${name}!`);
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error);
      localStorage.setItem('token', data.token);
      toast.success('Welcome to Vynkify!');
      router.push('/dashboard');
    } catch (err: any) {
    ...
    <main className="min-h-screen pt-32 pb-24 px-4 flex items-center justify-center relative overflow-hidden">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold font-heading mb-2 text-white">Create account</h1>
          <p className="text-[#a5a0c8] text-sm">Start Vynkifying your links today</p>
        </div>
        ...
        <div className="text-center mt-8">
          <p className="text-sm text-[#a5a0c8]">
            ...
            <Link href="/login" className="text-white font-medium hover:text-[#7f5fff] transition-colors">
              Sign in
            </Link>
          </p>
        </div>
